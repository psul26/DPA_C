...
for wave=1:IterationLimit
    ...
    for s=1:NS
        for candidate=1:NKEY
            wr(candidate,:,s) = wr(candidate,:,s) + w .* ref(s, candidate);
        end
    end
    ...
    for s=1:NS
        ...
        for candidate = 1:NKEY
            d = sqrt( (wave * w2 - w1 .^ 2) .* (wave * r2(s,candidate) - r1(s,candidate) .^ 2) );
            cc = (wave * wr(candidate,:,s) - w1 .* r1(s, candidate)) ./ d;
