...
%
% by Hideo Shimizu
% Copyright(C) 2009 by Toshiba Corporation. All rights reserved.

function makeref(outfile, filelist, IterationLimit)
...
h = zeros(1, 256); % hamming weight
c = zeros(1, 64);  % ciphertext
ref = zeros(64, IterationLimit, 8);

% initialize h as hamming weight
for i=0:255, h(i+1) = sum(bitget(i, [8 7 6 5 4 3 2 1])); end

for wave=1:IterationLimit
    filename = fscanf(flst, '%s', 1);
    % c(1:64) = each bit of ciphertext
    for i=1:16, c(i*4-3:i*4) = bitget(hex2dec(filename(73 + i)), [4 3 2 1]); end
    x0 = c(IP);       % x0 = calculate IP
