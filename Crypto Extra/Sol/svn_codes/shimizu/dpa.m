% CPA for random order
%
% by Hideo Shimizu
% Copyright(C) 2009 by Toshiba Corporation

function [successcount] = cpa(TRACEDIR, FILELIST, FLOG, VERBOSE, RATIO, FIRST, LAST, SIDE, SUBKEY, IterationThreshold, StabilityThreshold, IterationLimit)
...
% arrays
d1 = zeros(NKEY, len0, NS);
n1 = zeros(NS, NKEY);
d0 = zeros(NKEY, len0, NS);
n0 = zeros(NS, NKEY);
ref = zeros(NS, NKEY);
ccmax = zeros(1, NKEY);
maxkey = zeros(1, NS);
cipher = zeros(1, 64);       % message or ciphertext
...
for wave=1:IterationLimit
    ...
    for s=1:NS
        ...
        ref(s, :) = h(bitxor(so, lr0) + 1);
    end

    % sum part 2
    for s=1:NS
        for candidate=1:NKEY
            if ref(s, candidate) > 2
                d1(candidate,:,s) = d1(candidate,:,s) + w;
                n1(s, candidate) = n1(s, candidate) + 1;
            end
            if ref(s, candidate) < 2
                d0(candidate,:,s) = d0(candidate,:,s) + w;
                n0(s, candidate) = n0(s, candidate) + 1;
            end
        end
    end
    ...
    for s=1:NS
        ...
        for candidate = 1:NKEY
            cc = d1(candidate,:,s) / n1(s, candidate) - d0(candidate,:,s) / n0(s, candidate);
            ccmax(candidate) = max(abs(cc));
        end
