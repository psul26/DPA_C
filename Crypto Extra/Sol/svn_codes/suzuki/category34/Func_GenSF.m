...
function hw = Func_GenSF(indata,mode);%key 6bit, indata(1,64) 64bit

ip(1,:) = [58,50,42,34,26,18,10,2, ...
...
p(1,:) = [16, 7,20,21, ...
	...
	for subkey=1:1:64
		kb(1,1:6) = bitget(subkey-1,6:-1:1);
		for i=1:1:8
			sib(1,6*i-5:6*i) = xor(de(1,6*i-5:6*i),kb(1,1:6));
			sibt(1,1) = sib(1,6*i-5);
			sibt(1,2) = sib(1,6*i);
			sibt(1,3:6) = sib(1,6*i-4:6*i-1);
			sid = bit6_dec(1,1:6) * sibt(1,1:6)';
			so(1,i) = s(i,sid+1);
			pi(1,i*4-3:i*4) = bitget(so(1,i),4:-1:1);
		end
		...
		delta(1,1:32) = xor(po(1,1:32),dtl(1,1:32));
		delta(1,1:32) = xor(delta(1,1:32),dtr(1,1:32));
	
		for i=1:1:32
			b_p = p(1,i);
			s_num = fix((b_p-1)/4) + 1;
			if(mode==1)
			 	hw(s_num,subkey) = hw(s_num,subkey) + delta(1,i)*fanout(1,i);
			elseif(mode==0)
			 	hw(s_num,subkey) = hw(s_num,subkey) + delta(1,i);
			end
		end
