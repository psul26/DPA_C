...
function [MaxWaveNum StepWaveNum] = Func_DPAContest(WaveDir, FileListName, OutFileIndex, TargetRange, TraceCount,...
...
sf = Func_GenSF(fln,fc,SearchMode(1,1));

for i=1:1:st
	Status = sprintf('%s %02d %s%02d%s%d','Step:',i,'Target Key: K',sr(1,i)*15+1,'-S',sn(1,i))
	s = sn(1,i);
	if(sr(1,i)==1)
		ss = 6 - sum(bitget(bf(2,s),6:-1:1));
		[fwn skl] = Func_SearchKey(wd, fln, tr16, tc, it, cf, s, sm16, fkf, fk, bf, bd, sf);
		attack_log(i,:)=[16 s fwn ss];
		keylog16(s,:) = skl;
		fkf(2,s) = 1;
		fk(2,s) = skl(1,tc);
	...
	elseif(sr(1,i)==0)
		...
		[fwn skl] = Func_SearchKey(wd, fln, tr01, tc, it, cf, s, sm01, fkf, fk, bf, bd, sf);
		attack_log(i,:)=[1 s fwn ss];
		keylog01(s,:) = skl;
		fkf(1,s) = 1;
		fk(1,s) = skl(1,tc);
		[bf, bd] = Func_UpdateBC(fkf,fk);
...
for i=1:1:st
	...
	fprintf(fid3, 'SearchSpace=2^%d | ',attack_log(i,4));
	fprintf(fid3, 'FileNum=%02d \n',attack_log(i,3));
end
fclose(fid3);

MaxWaveNum = fc - cnt_s + 1;
StepWaveNum = attack_log(:,3)';