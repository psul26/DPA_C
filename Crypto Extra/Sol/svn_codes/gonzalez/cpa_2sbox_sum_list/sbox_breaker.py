...
# Copyright (C) 2009 Eloi Sanfelix ( eloi@riscure.com ) based on code by Florent Flament (florent.flament@telecom-paristech.fr)
#
...
class sbox_breaker:
	"""
	Provides method to break two consecutive sboxes, given some traces.
	"""
	__best_key= None
	__local_key = None
	__iter = 0
	...
	def __init__(self, sbox): # Sbox from 0 to 7
		"Initializes data arrays needed for the attack"
		self.__sbox = sbox
		self.__h = []
		self.__h2 = []
		self.__hw = []
	...
	def process(self, msg, trace):
		...
		for i in range(4096): #For each candidate
