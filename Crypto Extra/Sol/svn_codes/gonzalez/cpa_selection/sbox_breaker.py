...
class sbox_breaker:
	...
	def process(self, msg, trace):
		...
		Accumulates waves and hamming weights
		"""
	...
	def __get_differential(self):
		...
		for i in range(64):
			...
			#print "diff: ",len(self.__diff)," elements"
		return self.__diff
	...
	def get_key(self):
		...
		if self.__best_key == None:
			...
			marks = []
			for i in range(64):
                        	marks.append(max(map(abs,diff[i]))) #Add max correlation as the mark for this candidate
