...
%power traces folder name, csv format power trace location
%-------------------------------------------------------------------------
% PLEASE CHANGE THE FOLLOWING PATH WHERE THE CSV TRACE FILES ARE LOCATED
csvfolder = './traces'; 
%-------------------------------------------------------------------------
...
for i = 1 : no_of_curves
  ...
  for sbox=1:8
    ...
    for k=0:63
      ...
      pred2(k+1) = IV_tmp(1)*8 + IV_tmp(2)*4 + IV_tmp(3)*2 + IV_tmp(4);
    end
    pred_trace(i,(sbox-1)*64+1:sbox*64) = pred;
    pred_trace2(i,(sbox-1)*64+1:sbox*64) = pred2;
  end
end % loop over no_of_curves power traces

filename = fullfile(matfolder,'pred_trace_10000.mat');
save(filename,'pred_trace','pred_trace2');
filename = fullfile(matfolder,'measured_trace_10000.mat');
