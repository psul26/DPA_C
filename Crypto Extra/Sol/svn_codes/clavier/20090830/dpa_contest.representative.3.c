...
//  Univariate model:  consumption = a + b * hw01 + noise
//  where hw01 is the Hamming distance between left and right halves at the end of round 1

//  Estimation of coefficients a and b based on the data:
//
//             n * \sigma_i{(x_i * y_i)} - \sigma_i{x_i} * \sigma_i{y_i}
//  \hat{a} =  --------------------------------------------------------
//             n * \sigma_i{(x_i * x_i)} - \sigma_i{x_i} * \sigma_i{x_i}
//
//  \hat{b} =  n * \sigma_i{y_i} - \hat{a} * n * \sigma_i{x_i}
//
//  where x designates the Hamming distance hw01, and y designates the measurement (power consumption)
...
float  likelihood00(WORD32 nb_curve, BYTE* b6_key00)
{
  ...
  for (n0_id = 0; n0_id < nb_curve; n0_id++)
  {
    ...
    for (i = 0; i < 8; i++)
    {
      hw1[n0_id] += hw_01[id][i][b6_key00[i]];
    }

    sumx1 += hw1[n0_id];
    sumx2 += hw1[n0_id] * hw1[n0_id];
    sumxy += hw1[n0_id] * data_00[n0_id];
    sumy1 += data_00[n0_id];
  }
	
  a = ( sumxy - sumx1 * sumy1 / nb_curve ) / ( sumx2 - sumx1 * sumx1 / nb_curve );
	
  b = (sumy1 - a * sumx1) / nb_curve;
	
  for (n0_id = 0; n0_id < nb_curve; n0_id++)
  {
    prediction = a * hw1[n0_id] + b;
...
//  Univariate model:  consumption = a + b * hw15 + noise
//  where hw15 is the Hamming distance between left and right halves at the end of round 15
...
//  \hat{b} =  n * \sigma_i{y_i} - \hat{a} * n * \sigma_i{x_i}
//
//  where x designates the Hamming distance hw15, and y designates the measurement (power consumption)
...
float  likelihood16(WORD32 nb_curve, BYTE* b6_key16)
{
  ...
  for (n0_id = 0; n0_id < nb_curve; n0_id++)
  {
    ...
    for (i = 0; i < 8; i++)
    {
      hw1[n0_id] += hw_15[id][i][b6_key16[i]];
    }

    sumx1 += hw1[n0_id];
    sumx2 += hw1[n0_id] * hw1[n0_id];
    sumxy += hw1[n0_id] * data_16[n0_id];
    sumy1 += data_16[n0_id];
  }
