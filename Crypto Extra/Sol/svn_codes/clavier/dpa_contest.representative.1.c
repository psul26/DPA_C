...
#define T_15  14491
#define A_15  0.0010471951893003927
#define B_15  0.06000632840640302
...
#define NB_TRY_X1  75.0

#define NB_TRY_T2  45.0
#define NB_TRY_X2  42.0
...
static  BYTE  hw00[NB_CURVE];
static  BYTE  hw01[NB_CURVE][8][64];
static  BYTE  hw15[NB_CURVE][8][64];
static  BYTE  hw16[NB_CURVE];

static  float data00[NB_CURVE];
static  float data15[NB_CURVE];
static  float data16[NB_CURVE];
...
float  likelihood00(WORD32 nb_curve, BYTE* b6_key00)
{
  ...
  for (n0_id = 0; n0_id < nb_curve; n0_id++)
  {
    ...
    for (i = 0; i < 8; i++)
    {
      hw_01 += hw01[id][i][b6_key00[i]];
    }

    hw_00 = hw00[id];

    prediction = A_00 * hw_01 + B_00 * hw_00 + C_00;
    error = data00[id] - prediction;
    sum += (error * error);
  }
...
float  likelihood16(WORD32 nb_curve, BYTE* b6_key16)
{
  ...
  for (n0_id = 0; n0_id < nb_curve; n0_id++)
  {
    ...
    for (i = 0; i < 8; i++)
    {
      hw_15 += hw15[id][i][b6_key16[i]];
    }

    hw_16 = hw16[id];

    prediction = A_16 * hw_15 + B_16 * hw_16 + C_16;
    error = data16[id] - prediction;
    sum += (error * error);
    
    prediction = A_15 * hw_15 + B_15;
    error = data15[id] - prediction;
    sum += (error * error);
  }
...
//  INT32   temp_key_2[64];
    ...
    */
    ...
    for (i = 0; i < 8; i++)
    {
      for (k = 0; k < 64; k++)
      {
        ...
        for (j = 0; j < 4; j++)
        {
          hw01[id][i][k] += ((b4_y >> (3 - j)) & 1) ^ b1_R00[inv_P[4 * i + j] - OFFSET] ^ b1_L00[inv_P[4 * i + j] - OFFSET];
        }
      }
    }
    ...
    */
    ...
    for (i = 0; i < 8; i++)
    {
      for (k = 0; k < 64; k++)
      {
        ...
        for (j = 0; j < 4; j++)
        {
          hw15[id][i][k] += ((b4_y >> (3 - j)) & 1) ^ b1_R16[inv_P[4 * i + j] - OFFSET] ^ b1_L16[inv_P[4 * i + j] - OFFSET];
        }
      }
    }
  ...
  */
  ...
  for (id = 0; id < NB_CURVE; id++)
  {
    ...
    fread(&(data00[id]), sizeof(float), 1, pfd_curve);

    fseek(pfd_curve, 164 + T_15 * sizeof(float), SEEK_SET);
    fread(&(data15[id]), sizeof(float), 1, pfd_curve);

    fseek(pfd_curve, 164 + T_16 * sizeof(float), SEEK_SET);
    fread(&(data16[id]), sizeof(float), 1, pfd_curve);
...
/*
    ...
    fread(&data00, sizeof(float), NB_CURVE, pfd_curve);
    fclose(pfd_curve);

    sprintf(sz_file_name,"./sample_%05lu.dat", (WORD32) T_15);
    if ((pfd_curve = fopen(sz_file_name, "r")) == NULL)
    {
      printf("\nERROR : can not open file named \"%s\" for reading\n\n", sz_file_name);
      exit(-1);
    }
    fread(&data15, sizeof(float), NB_CURVE, pfd_curve);
    fclose(pfd_curve);
    ...
    fread(&data16, sizeof(float), NB_CURVE, pfd_curve);
    fclose(pfd_curve);
*/
  ...
  */
  ...
  {  
    /*
      Generate the index of curves (random permutation)
      -------------------------------------------------
    */
    ...
    for (n0_id = 0; n0_id < NB_CURVE; n0_id++)
    {
      pos = n0_id + rand() % (NB_CURVE - n0_id);
      tmp = id_list[n0_id];
      id_list[n0_id] = id_list[pos];
      id_list[pos] = tmp;
    }
